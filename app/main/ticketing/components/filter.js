...
const Filter = ({ handleSearchChange, statusFilter, setStatusFilter, statusCounts }) => {
  return (
    <div className='mb-4'>
      <div className="flex gap-2 items-center px-5 sm:px-0 pb-3 sm:max-w-64">
        <Input
          placeholder="Search by Ticket ID / Customer ID"
      ...
      <div className="flex flex-wrap gap-2 px-6 sm:px-0 pb-4">
        ...
        <button 
          ...
          className={`p-1 px-2 text-xs shadow-sm rounded-sm sm:rounded-lg text-white hover:bg-amtorange ${statusFilter == 'Open' ? 'bg-amtorange/80' : 'bg-slate-300'}`}
        >
